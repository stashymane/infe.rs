...
def export_yolo26n_face(weights_path: Path, imgsz: int, work_dir: Path) -> tuple[Path, Path]:
...
def main() -> int:
    ...
    imgsz = int(manifest.get("imgsz", 192))
    if imgsz % 32 != 0:
    ...
    with tempfile.TemporaryDirectory(prefix="yolo26n-face-export-") as tmp:
        ...
        xnnpack_pte, vulkan_pte = export_yolo26n_face(weights_path, imgsz, work_dir)
