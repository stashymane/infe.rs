...
public class Backend : AutoCloseable {
     ...
     */
    public suspend fun availableDevices(): List<Device> = withContext(Dispatchers.Default) {
        gate.ensureOpen()
     ...
     */
    public suspend fun loadModel(path: Path, config: BackendConfig): ModelSession = withContext(Dispatchers.IO) {
        withFfiErrors {
            gate.ensureOpen()
