...
import dev.stashy.infers.process
import dev.stashy.infers.toHardwareBuffer
...
class GpuHardwareBufferInstrumentedTest {
    @Test
    fun gpuProcessHardwareBuffer() =
        runTest {
            val context = runCatching { GpuContext(Device.gpu(0u)) }.getOrNull()
            assumeTrue("No Vulkan device available", context != null)
    ...
    private fun createGpuReadableRgbBuffer(): AndroidHardwareBuffer =
